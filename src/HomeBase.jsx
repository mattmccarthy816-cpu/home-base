...
async function sheetsAppend(sheetName, rowData) {
  const url = `${APPS_SCRIPT_URL}?action=appendRow&sheet=${encodeURIComponent(sheetName)}&data=${encodeURIComponent(JSON.stringify(rowData))}`;
...
async function sheetsUpdate(sheetName, matchCol, matchVal, newData) {
  const url = `${APPS_SCRIPT_URL}?action=updateRow&sheet=${encodeURIComponent(sheetName)}&matchCol=${encodeURIComponent(matchCol)}&matchVal=${encodeURIComponent(String(matchVal))}&data=${encodeURIComponent(JSON.stringify(newData))}`;
  const r = await fetch(url);
  const json = await r.json();
  if (!json.success) throw new Error(json.error);
  return json.data;
...
async function sheetsDelete(sheetName, matchCol, matchVal) {
  const url = `${APPS_SCRIPT_URL}?action=deleteRow&sheet=${encodeURIComponent(sheetName)}&matchCol=${encodeURIComponent(matchCol)}&matchVal=${encodeURIComponent(String(matchVal))}`;
  const r = await fetch(url);
  const json = await r.json();
  if (!json.success) throw new Error(json.error);
  return json.data;
...
const DEFAULT_ACTIVITY_TYPES = ["Local","Trip","Outdoor","Entertainment","Food","Other"];
...
function Sparkline({ data, color="#6ee7b7" }) {
  ...
  return <svg width={w} height={h} style={{display:"block"}}><polyline points={pts} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><circle cx={lp[0]} cy={lp[1]} r="3" fill={color}/></svg>;
}

function SaveBadge({ saving, saved, error }) {
  if (saving) return <span style={{fontSize:11,color:"#9ca3af"}}>Saving…</span>;
  if (error)  return <span style={{fontSize:11,color:"#f87171"}}>⚠ Save failed</span>;
  if (saved)  return <span style={{fontSize:11,color:"#6ee7b7"}}>✓ Saved</span>;
  return null;
}

const primaryBtn = { background:"#6ee7b7",color:"#0f1c14",border:"none",borderRadius:8,padding:"8px 18px",cursor:"pointer",fontWeight:700,fontSize:13 };
const ghostBtn   = { background:"transparent",color:"#9ca3af",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"8px 14px",cursor:"pointer",fontSize:13 };
const navBtn     = { background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,color:"#f0e6d3",width:28,height:28,cursor:"pointer",fontSize:16,padding:0 };
const IS = { background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:7,color:"#f0e6d3",padding:"8px 11px",fontSize:13,outline:"none",width:"100%",boxSizing:"border-box" };

// ── WEEKLY PLAN ───────────────────────────────────────────────────────────────
function WeeklyPlan() {
  const now = new Date();
  const [year,month,setYear,setMonth] = [useState(now.getFullYear()),useState(now.getMonth())].flatMap(s=>s);
  ...
  useEffect(()=>{
    sheetsRead("WeeklyPlan").then(rows=>{
      const map={};
      rows.forEach(r=>{ map[r.Date]={mattLoc:r.MattLocation||"",aliceLoc:r.AliceLocation||"",dinner:r.Dinner||"",appts:r.Appointments||""}; });
      setPlan(map);
    }).catch(()=>{});
  },[]);
  ...
  const prevM=()=>{ if(month===0){setYear(y=>y-1);setMonth(11);}else setMonth(m=>m-1); setEditing(null); };
  const nextM=()=>{ if(month===11){setYear(y=>y+1);setMonth(0);}else setMonth(m=>m+1); setEditing(null); };
  const locIcon=loc=>loc==="Office"?"🏢":loc==="Home"?"🏠":"";
  const editEntry = plan[editing]||{mattLoc:"",aliceLoc:"",dinner:"",appts:""};

  const saveDay=async(ds)=>{
    const e=plan[ds]||{};
    const row={Date:ds,MattLocation:e.mattLoc||"",AliceLocation:e.aliceLoc||"",Dinner:e.dinner||"",Appointments:e.appts||""};
    setSv({saving:true,saved:false,error:false});
    try{
      const res=await sheetsUpdate("WeeklyPlan","Date",ds,row);
    ...
    }catch(err){ console.error(err); setSv({saving:false,saved:false,error:true}); }
    setEditing(null);
  };
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:0,fontSize:20}}>Monthly Plan</h2>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={prevM} style={navBtn}>‹</button>
          <span style={{color:"#f0e6d3",fontSize:12,fontWeight:600,minWidth:120,textAlign:"center"}}>{monthLabel}</span>
          <button onClick={nextM} style={navBtn}>›</button>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:2}}>
        {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=><div key={d} style={{textAlign:"center",fontSize:9,color:"#6b7280",paddingBottom:3}}>{d}</div>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        {days.map((d,i)=>{
          if(!d) return <div key={`e${i}`}/>;
          const ds=fmt(d), isToday=ds===todayDS, isEdit=editing===ds, entry=plan[ds]||{};
          return (
            <div key={ds} onClick={()=>setEditing(isEdit?null:ds)}
              style={{background:isEdit?"rgba(110,231,183,0.2)":isToday?"rgba(110,231,183,0.1)":"rgba(255,255,255,0.04)",
                border:`1px solid ${isEdit?"rgba(110,231,183,0.7)":isToday?"rgba(110,231,183,0.4)":"rgba(255,255,255,0.07)"}`,
                borderRadius:7,padding:"5px 4px",cursor:"pointer",minHeight:62,transition:"all 0.15s"}}>
              <div style={{fontSize:12,fontWeight:700,color:isToday?"#6ee7b7":"#f0e6d3",marginBottom:2}}>{d.getDate()}</div>
              {entry.mattLoc  && <div style={{fontSize:9,color:"#93c5fd"}}>{locIcon(entry.mattLoc)} M</div>}
              {entry.aliceLoc && <div style={{fontSize:9,color:"#f9a8d4"}}>{locIcon(entry.aliceLoc)} A</div>}
              {entry.dinner   && <div style={{fontSize:9,color:"#fcd34d",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>🍽 {entry.dinner}</div>}
              {entry.appts    && <div style={{fontSize:9,color:"#fb923c",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>📌 {entry.appts}</div>}
            </div>
          );
        })}
      </div>
      ...
      {editing && (
        <div style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(110,231,183,0.25)",borderRadius:12,padding:16,marginTop:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <span style={{color:"#f0e6d3",fontWeight:600,fontSize:14}}>
              {new Date(editing+"T12:00:00").toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}
            </span>
            <SaveBadge {...sv}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            {[["Matt","mattLoc","#93c5fd"],["Alice","aliceLoc","#f9a8d4"]].map(([name,key,color])=>(
              <div key={key}>
                <label style={{fontSize:11,color,display:"block",marginBottom:5,fontWeight:600}}>{name}'s Location</label>
                <div style={{display:"flex",gap:6}}>
                  {[["🏢","Office"],["🏠","Home"]].map(([icon,val])=>{
                    const active=editEntry[key]===val;
                    return <button key={val} onClick={()=>setPlan(p=>({...p,[editing]:{...(p[editing]||{}),[key]:active?"":val}}))}
                      style={{flex:1,background:active?`${color}22`:"rgba(255,255,255,0.06)",border:`1px solid ${active?color:"rgba(255,255,255,0.1)"}`,
                        borderRadius:8,color:active?color:"#6b7280",padding:"8px 4px",cursor:"pointer",fontSize:13,fontWeight:active?700:400}}>
                      {icon} {val}
                    </button>;
                  })}
                </div>
          ...
          {[["🍽 Dinner","dinner"],["📌 Appointments","appts"]].map(([label,key])=>(
            <div key={key} style={{marginBottom:9}}>
              <label style={{fontSize:11,color:"#9ca3af",display:"block",marginBottom:3}}>{label}</label>
              <input value={editEntry[key]||""} onChange={e=>setPlan(p=>({...p,[editing]:{...(p[editing]||{}),[key]:e.target.value}}))}
                onKeyDown={e=>e.key==="Enter"&&saveDay(editing)} style={IS}/>
            </div>
          ))}
          <div style={{display:"flex",gap:8,marginTop:4}}>
            <button onClick={()=>saveDay(editing)} style={primaryBtn}>Save</button>
...
function Todos() {
  ...
  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 14px",fontSize:20}}>Today's Tasks</h2>
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Add a task…" style={{...IS,flex:1}}/>
...
// ── FITNESS ───────────────────────────────────────────────────────────────────
function Fitness() {
  const now=new Date(), todayName=fullDayNames[now.getDay()];
  const [year,setYear]=useState(now.getFullYear());
  const [month,setMonth]=useState(now.getMonth());
  const [fitnessData,setFitnessData]=useState(MOCK_FITNESS);
  ...
  useEffect(()=>{
    sheetsRead("Fitness").then(rows=>{ if(rows.length){ setFitnessData(rows.map(r=>({day:r.Day,group:r.MuscleGroup,exercises:r.Exercises}))); const mon=rows.find(r=>r.Day==="Monday"); if(mon)setMeds({am:mon.AMmeds||"",pm:mon.PMmeds||""}); }}).catch(()=>{});
    sheetsRead("HealthLog").then(rows=>setHealthLog(rows.map(r=>({date:r.Date,mattWeight:parseFloat(r.MattWeight)||null,mattBPSys:parseInt(r.MattBPSys)||null,mattBPDia:parseInt(r.MattBPDia)||null,aliceBPSys:parseInt(r.AliceBPSys)||null,aliceBPDia:parseInt(r.AliceBPDia)||null})))).catch(()=>{});
  },[]);
  ...
  const prevM=()=>{ if(month===0){setYear(y=>y-1);setMonth(11);}else setMonth(m=>m-1); setEditing(null); };
  const nextM=()=>{ if(month===11){setYear(y=>y+1);setMonth(0);}else setMonth(m=>m+1); setEditing(null); };
  const todayFit=fitnessData.find(f=>f.day===todayName)||fitnessData[0];
  const hasData=ds=>healthLog.some(h=>h.date===ds&&(h.mattWeight||h.mattBPSys||h.aliceBPSys));

  const openEdit=ds=>{ if(editing===ds){setEditing(null);return;} const ex=healthLog.find(h=>h.date===ds)||{}; setLogForm({mattWeight:ex.mattWeight||"",mattBPSys:ex.mattBPSys||"",mattBPDia:ex.mattBPDia||"",aliceBPSys:ex.aliceBPSys||"",aliceBPDia:ex.aliceBPDia||""}); setEditing(ds); };

  const saveLog=async()=>{
    if(!editing)return;
    const entry={date:editing,mattWeight:parseFloat(logForm.mattWeight)||null,mattBPSys:parseInt(logForm.mattBPSys)||null,mattBPDia:parseInt(logForm.mattBPDia)||null,aliceBPSys:parseInt(logForm.aliceBPSys)||null,aliceBPDia:parseInt(logForm.aliceBPDia)||null};
    setHealthLog(h=>{ const idx=h.findIndex(x=>x.date===editing); if(idx>=0){const n=[...h];n[idx]=entry;return n;} return [...h,entry].sort((a,b)=>a.date.localeCompare(b.date)); });
    setSv({saving:true,saved:false,error:false});
    try{
      ...
      const res=await sheetsUpdate("HealthLog","Date",editing,row);
  ...
  const as=healthLog.filter(h=>h.aliceBPSys).map(h=>h.aliceBPSys);
  const ad=healthLog.filter(h=>h.aliceBPDia).map(h=>h.aliceBPDia);

  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 12px",fontSize:20}}>Fitness & Health</h2>
      <div style={{background:"rgba(110,231,183,0.1)",border:"1px solid rgba(110,231,183,0.25)",borderRadius:12,padding:14,marginBottom:12}}>
        <div style={{fontSize:11,color:"#6ee7b7",textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>Today — {todayName}</div>
        <div style={{fontSize:17,fontWeight:700,color:"#f0e6d3",marginBottom:3}}>{todayFit?.group}</div>
        <div style={{fontSize:12,color:"#9ca3af"}}>{todayFit?.exercises}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
        {[["☀️ AM",meds.am],["🌙 PM",meds.pm]].map(([l,v])=>(
          <div key={l} style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:11}}>
            <div style={{fontSize:11,color:"#fbbf24",marginBottom:4,fontWeight:600}}>{l}</div>
      ...
      <div style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          <span style={{fontSize:12,color:"#9ca3af"}}>Tap a day to log measurements</span>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={prevM} style={navBtn}>‹</button>
            <span style={{color:"#f0e6d3",fontSize:11,fontWeight:600}}>{monthLabel}</span>
            <button onClick={nextM} style={navBtn}>›</button>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:2}}>
          {["M","T","W","T","F","S","S"].map((d,i)=><div key={i} style={{textAlign:"center",fontSize:9,color:"#6b7280"}}>{d}</div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
          {days.map((d,i)=>{
            if(!d)return<div key={`e${i}`}/>;
            const ds=fmt(d),isToday=ds===todayStr(),logged=hasData(ds),isEdit=editing===ds;
            return <div key={ds} onClick={()=>openEdit(ds)}
              style={{background:isEdit?"rgba(110,231,183,0.2)":logged?"rgba(110,231,183,0.07)":"rgba(255,255,255,0.04)",
                border:`1px solid ${isEdit?"rgba(110,231,183,0.6)":isToday?"rgba(110,231,183,0.4)":"rgba(255,255,255,0.07)"}`,
                borderRadius:6,padding:"5px 3px",textAlign:"center",cursor:"pointer",minHeight:30}}>
              <div style={{fontSize:11,fontWeight:700,color:isToday?"#6ee7b7":"#f0e6d3"}}>{d.getDate()}</div>
              {logged&&<div style={{fontSize:7,color:"#6ee7b7",marginTop:1}}>●</div>}
            </div>;
          })}
        </div>
      </div>
      ...
      {editing&&(
        <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:14,marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{color:"#f0e6d3",fontWeight:600,fontSize:13}}>{new Date(editing+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</span>
            <SaveBadge {...sv}/>
          </div>
          <div style={{fontSize:12,color:"#93c5fd",fontWeight:600,marginBottom:6}}>Matt</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7,marginBottom:12}}>
            {[["Weight","mattWeight"],["BP Sys","mattBPSys"],["BP Dia","mattBPDia"]].map(([l,k])=>(
              <div key={k}><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:3}}>{l}</label>
              <input type="number" value={logForm[k]} onChange={e=>setLogForm(f=>({...f,[k]:e.target.value}))} style={IS}/></div>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={saveLog} style={primaryBtn}>Save</button>
            <button onClick={()=>setEditing(null)} style={ghostBtn}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div>
        <div style={{fontSize:12,color:"#9ca3af",marginBottom:8,fontWeight:600}}>Trends</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[["⚖️ Matt Weight",mw,"#6ee7b7","lbs"],["❤️ Matt BP Sys",ms,"#f87171","mmHg"],["🩺 Matt BP Dia",md,"#fb923c","mmHg"],["❤️ Alice BP Sys",as,"#f9a8d4","mmHg"],["🩺 Alice BP Dia",ad,"#e879f9","mmHg"]].map(([label,data,color,unit])=>(
            <div key={label} style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:11}}>
              ...
              <div style={{fontSize:18,fontWeight:700,color,marginBottom:3}}>{data.length?data.at(-1):<span style={{color:"#4b5563"}}>—</span>}{data.length?<span style={{fontSize:10,color:"#6b7280",marginLeft:3}}>{unit}</span>:null}</div>
...
function Habits() {
  ...
  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 14px",fontSize:20}}>Daily Habits</h2>
      ...
      <div style={{display:"flex",gap:8,marginTop:14}}>
        <input value={newHabit} onChange={e=>setNewHabit(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Add a habit…" style={{...IS,flex:1}}/>
        <button onClick={add} style={primaryBtn}>Add</button>
      </div>
    </div>
  );
...
function Restaurants() {
  ...
  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 14px",fontSize:20}}>Restaurants</h2>
      <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:14,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
          <input value={zip} onChange={e=>setZip(e.target.value)} placeholder="ZIP code" style={IS}/>
          <input value={cuisine} onChange={e=>setCuisine(e.target.value)} placeholder="Cuisine type" style={IS}/>
        </div>
        <button onClick={search} disabled={searching} style={{...primaryBtn,width:"100%",opacity:searching?0.6:1}}>{searching?"Searching…":"Search Google Places"}</button>
        {results.length>0&&<div style={{marginTop:10}}>{results.map((r,i)=>(
          <div key={i} style={{background:"rgba(255,255,255,0.06)",borderRadius:8,padding:"9px 12px",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={()=>saveToSheet(r)} style={{...ghostBtn,fontSize:11,padding:"5px 10px"}}>+ Save</button>
          </div>
        ))}</div>}
      </div>
...
function Movies() {
  ...
  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 14px",fontSize:20}}>Movies</h2>
      <div style={{background:"rgba(139,92,246,0.1)",border:"1px solid rgba(139,92,246,0.25)",borderRadius:12,padding:14,marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&getAiRecs()} placeholder="e.g. 'funny' or 'like Dune'" style={{...IS,flex:1}}/>
        ...
        {aiRecs.length>0&&<div style={{marginTop:10,display:"flex",flexDirection:"column",gap:7}}>{aiRecs.map((r,i)=>(
          <div key={i} style={{background:"rgba(139,92,246,0.15)",borderRadius:8,padding:"9px 12px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            ...
            <button onClick={()=>addToList(r)} style={{...ghostBtn,fontSize:11,padding:"4px 9px",marginLeft:8,flexShrink:0}}>+ Add</button>
          </div>
        ))}</div>}
      </div>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {statuses.map(s=><button key={s} onClick={()=>setFilter(s)} style={{background:filter===s?"#6ee7b7":"rgba(255,255,255,0.07)",color:filter===s?"#0f1c14":"#9ca3af",border:"none",borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:filter===s?700:400}}>{s}</button>)}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {filtered.map((m,i)=>(
...
function Books() {
  ...
  return (
    <div>
      <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:"0 0 14px",fontSize:20}}>Books</h2>
      ...
      <div style={{display:"flex",gap:5,marginBottom:12,flexWrap:"wrap"}}>
        {allFilters.map(f=><button key={f} onClick={()=>setFilter(f)} style={{background:filter===f?"#6ee7b7":"rgba(255,255,255,0.07)",color:filter===f?"#0f1c14":"#9ca3af",border:"none",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:filter===f?700:400}}>{f}</button>)}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {filtered.map((b,i)=>{
          ...
          if(editIdx===ri&&editBook) return (
            <div key={i} style={{background:"rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px"}}>
              {[["Title","title"],["Author","author"],["Category","category"]].map(([l,k])=>(
                ...
                <input value={editBook[k]} onChange={e=>setEditBook(eb=>({...eb,[k]:e.target.value}))} style={IS}/></div>
              ))}
              <div style={{marginBottom:10}}><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:2}}>Status</label>
              <select value={editBook.status} onChange={e=>setEditBook(eb=>({...eb,status:e.target.value}))} style={{width:"100%",background:"#1f2937",border:"1px solid rgba(255,255,255,0.12)",borderRadius:6,color:"#f0e6d3",padding:"7px 10px",fontSize:13,outline:"none"}}>{statuses.map(s=><option key={s}>{s}</option>)}</select></div>
              <div style={{display:"flex",gap:8}}><button onClick={saveEdit} style={primaryBtn}>Save</button><button onClick={()=>setEditIdx(null)} style={ghostBtn}>Cancel</button></div>
            </div>
          );
          return (
            <div key={i} style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:"11px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      {adding?(
        <div style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:14,marginTop:12}}>
          {[["Title","title"],["Author","author"],["Category","category"]].map(([l,k])=>(
            ...
            <input value={newBook[k]} onChange={e=>setNewBook(n=>({...n,[k]:e.target.value}))} style={IS}/></div>
          ))}
          <div style={{marginBottom:10}}><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>Status</label>
          <select value={newBook.status} onChange={e=>setNewBook(n=>({...n,status:e.target.value}))} style={{width:"100%",background:"#1f2937",border:"1px solid rgba(255,255,255,0.12)",borderRadius:6,color:"#f0e6d3",padding:"7px 10px",fontSize:13,outline:"none"}}>{statuses.map(s=><option key={s}>{s}</option>)}</select></div>
          <div style={{display:"flex",gap:8}}><button onClick={add} style={primaryBtn}>Add</button><button onClick={()=>setAdding(false)} style={ghostBtn}>Cancel</button></div>
        </div>
...
function Activities() {
  ...
  const [actTypes,setActTypes]=useState(DEFAULT_ACTIVITY_TYPES);
  const [filter,setFilter]=useState("All");
  const [adding,setAdding]=useState(false), [editIdx,setEditIdx]=useState(null);
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <h2 style={{color:"#f0e6d3",fontFamily:"'Playfair Display',serif",margin:0,fontSize:20}}>Activities & Trips</h2>
        <button onClick={()=>setManagingTypes(m=>!m)} style={{...ghostBtn,fontSize:11,padding:"5px 10px"}}>{managingTypes?"Done":"⚙ Types"}</button>
      </div>
      ...
      {managingTypes&&(
        <div style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:14,marginBottom:14}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
            {actTypes.map(t=><span key={t} style={{background:"rgba(255,255,255,0.08)",borderRadius:20,padding:"4px 10px",fontSize:12,color:"#d1d5db",display:"flex",alignItems:"center",gap:6}}>{t}<span onClick={()=>removeType(t)} style={{cursor:"pointer",color:"#6b7280",fontSize:14}}>×</span></span>)}
          </div>
          <div style={{display:"flex",gap:8}}>
            <input value={newType} onChange={e=>setNewType(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addType()} placeholder="New type…" style={{...IS,flex:1}}/>
            <button onClick={addType} style={primaryBtn}>Add</button>
          </div>
        </div>
      )}

      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
        {statuses.map(s=><div key={s} style={{background:"rgba(255,255,255,0.05)",borderRadius:10,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:statusColor[s]}}>{activities.filter(a=>a.status===s).length}</div><div style={{fontSize:10,color:"#6b7280",marginTop:2}}>{s}</div></div>)}
      </div>
      <div style={{display:"flex",gap:5,marginBottom:12,flexWrap:"wrap"}}>
        {allFilters.map(f=><button key={f} onClick={()=>setFilter(f)} style={{background:filter===f?"#6ee7b7":"rgba(255,255,255,0.07)",color:filter===f?"#0f1c14":"#9ca3af",border:"none",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:11,fontWeight:filter===f?700:400}}>{f}</button>)}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {filtered.map((a,i)=>{
          ...
          if(editIdx===ri&&editAct) return (
            <div key={i} style={{background:"rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px"}}>
              {[["Name","name"],["Date","date"]].map(([l,k])=>(
                <div key={k} style={{marginBottom:7}}><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:2}}>{l}</label>
                <input value={editAct[k]} onChange={e=>setEditAct(ea=>({...ea,[k]:e.target.value}))} style={IS}/></div>
              ))}
              {[["Type","type",actTypes],["Status","status",statuses]].map(([l,k,opts])=>(
                <div key={k} style={{marginBottom:8}}><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:2}}>{l}</label>
                <select value={editAct[k]} onChange={e=>setEditAct(ea=>({...ea,[k]:e.target.value}))} style={{width:"100%",background:"#1f2937",border:"1px solid rgba(255,255,255,0.12)",borderRadius:6,color:"#f0e6d3",padding:"7px 10px",fontSize:13,outline:"none"}}>{opts.map(o=><option key={o}>{o}</option>)}</select></div>
              ))}
              <div style={{display:"flex",gap:8}}><button onClick={saveEdit} style={primaryBtn}>Save</button><button onClick={()=>setEditIdx(null)} style={ghostBtn}>Cancel</button></div>
            </div>
          );
      ...
      {adding?(
        <div style={{background:"rgba(255,255,255,0.05)",borderRadius:12,padding:14,marginTop:12}}>
          {[["Name","name"],["Date (optional)","date"]].map(([l,k])=>(
            <div key={k} style={{marginBottom:8}}><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>{l}</label>
            <input value={newAct[k]} onChange={e=>setNewAct(n=>({...n,[k]:e.target.value}))} style={IS}/></div>
          ))}
          {[["Type","type",actTypes],["Status","status",statuses]].map(([l,k,opts])=>(
            <div key={k} style={{marginBottom:8}}><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:3}}>{l}</label>
            <select value={newAct[k]} onChange={e=>setNewAct(n=>({...n,[k]:e.target.value}))} style={{width:"100%",background:"#1f2937",border:"1px solid rgba(255,255,255,0.12)",borderRadius:6,color:"#f0e6d3",padding:"7px 10px",fontSize:13,outline:"none"}}>{opts.map(o=><option key={o}>{o}</option>)}</select></div>
          ))}
          <div style={{display:"flex",gap:8}}><button onClick={add} style={primaryBtn}>Add</button><button onClick={()=>setAdding(false)} style={ghostBtn}>Cancel</button></div>
        </div>
...
const SECTION_MAP={weekly:WeeklyPlan,todos:Todos,fitness:Fitness,habits:Habits,restaurants:Restaurants,movies:Movies,books:Books,activities:Activities};
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:5}}>
          {TABS.map(tab=>{
            ...
            return <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
              style={{background:isActive?"linear-gradient(135deg,rgba(110,231,183,0.25),rgba(167,139,250,0.15))":"rgba(255,255,255,0.04)",border:isActive?"1px solid rgba(110,231,183,0.4)":"1px solid rgba(255,255,255,0.07)",borderRadius:9,padding:"7px 4px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,transition:"all 0.15s",transform:isActive?"scale(1.03)":"scale(1)"}}>
              <span style={{fontSize:17,lineHeight:1}}>{tab.icon}</span>
              <span style={{fontSize:10,fontWeight:isActive?700:400,color:isActive?"#6ee7b7":"#6b7280"}}>{tab.label}</span>
            </button>;
          })}
        </div>
      </div>
      <div style={{padding:"16px 14px 48px",maxWidth:820,margin:"0 auto"}}>
        <ActiveSection/>
      </div>
    </div>
  );
